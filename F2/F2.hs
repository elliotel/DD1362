...
class Evol a where
    ...
    distanceMatrix [] = []
    distanceMatrix list = [(name a, name b, distance a b) | (a, i) <- listWithIndexes, (b, j) <- listWithIndexes, i <= j]
        where
        listWithIndexes = zip list [0..]
...
data Profile = Prof [[(Char, Double)]] String Int String
...
main :: IO ()
main = do
    -- Testing for the sequence part of the assignment begins here.
    let dnaMolecule = string2seq "A" "AAGTCCGTAGA"
    let dnaMolecule2 = string2seq "B" "AAGTCAGTAGA"
    let dnaMolecule3 = string2seq "C" "GGAGGGAGGAG"
    let proteinMolecule = string2seq "A" "AAGTCEGTAGA"
    let proteinMolecule2 = string2seq "B" "AAETCAGTAEA"
    let proteinMolecule3 = string2seq "C" "EEEEEEEEEEE"
    print dnaMolecule
    print dnaMolecule2
    print dnaMolecule3
    ...
    putStrLn ""
    -- Testing for the profile part of the assignment begins here.
    let dnaMolecule4 = string2seq "D" "GGTACAGATAC"
    ...
    let proteinMolecule4 = string2seq "D" "GETACAIATDC"
    ...
    print (distanceMatrix [dnaMolecule, dnaMolecule2, dnaMolecule3, dnaMolecule4])
    putStrLn ""
    print (distanceMatrix [proteinMolecule, proteinMolecule2, proteinMolecule3, proteinMolecule4])
    putStrLn ""
    print (distanceMatrix [dnaMolecule, dnaMolecule2])
    putStrLn ""
    print (distanceMatrix [proteinMolecule, proteinMolecule2])
    putStrLn ""
    print (distanceMatrix [dnaMolecule])
    putStrLn ""
    print (distanceMatrix [proteinMolecule])
    putStrLn ""
    --print (distanceMatrix ([] :: [MolSeq]))
    putStrLn ""
    --print (distanceMatrix ([] :: [Profile]))
    putStrLn ""
    print (distanceMatrix [dnaMolecule3, dnaMolecule2, dnaMolecule])
    putStrLn ""
    print (distanceMatrix [proteinMolecule3, proteinMolecule2, proteinMolecule])
    putStrLn ""
            ...
            in  
                if molType /= seqType seq2
                    then error "Invalid argument: Comparing DNA with Protein"
                             ...
                             threshold = if isDNA then 0.74 else 0.94
                             result 
...
makeProfileMatrix mols = matrix
    where
        t = seqType (head mols) -- Sparar typen av Molseq, dvs DNA eller Protein
        defaults =
            if t == "DNA" then
        ...
        seqs = map seqSequence mols -- Kör funktionen seqSequence på varje element i s1 och sparar som ny lista, seqs, vilken alltså blir en lista med alla sekvenser som strängar.
        ...
        func x = (head x, fromIntegral (length x) / fromIntegral (length mols)) -- Tar en lista och returnerar en tuple som innehåller det första elementet i listan, samt listans längd. Exempelvis blir [5, 2, 3, 4] till (5, 4)
        ...
        undividedMatrix = map (sort . combineTuples) elementsByPosition {- Kör combineTuples samt sorterar resultatet på varje lista i elementsByPosition, sorteringsex: [('A',2),('G',1),('C',0)] blir [('A',2),('C',0),('G',1)].
...
profileFrequency (Prof matrix _ _ _) pos char = findFrequency (matrix !! pos) -- Tar listan på position pos i matrisen matrix
    where
        findFrequency :: [(Char, Double)] -> Double
        findFrequency [] = error "Character not part of any sequence!"
        findFrequency ((c, freq):xs) -- Går igenom varje tuple i listan
            | c == char = freq -- Om bokstaven stämmer med char så tar den det andra värdet ur tuplen och returnerar det som frekvensen
...
profileDistance (Prof matrix1 variation1 _ _) (Prof matrix2 variation2 _ _)
    ...
    | otherwise = distance
    where
        sumTuples :: [(Char, Double)] -> [(Char, Double)] -> Double
        sumTuples [] [] = 0.0
        ...
        sumTuples ((_, freq1):xs1) ((_, freq2):xs2) = abs (freq1 - freq2) + sumTuples xs1 xs2

        sumFrequency :: [[(Char, Double)]] -> [[(Char, Double)]] -> Double
        sumFrequency [] [] = 0.0
